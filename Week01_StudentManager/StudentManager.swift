...
class StudentManager {
    ...
    init(dbName: String) {
        studentList = [Student]()
        self.dbName = dbName
        sqliteHelper = SQLiteHelper.getInstance(dbName: self.dbName, tableName: "StudentManager")
    }
    
    public func fetchStudentFromDb()
    {
        studentList = sqliteHelper.selectStudent(selectQuery: "SELECT * FROM " + sqliteHelper.getTableName() + "")
    }
